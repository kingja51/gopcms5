...
- [ ] 셀프 탈퇴 → P10-4 의 탈퇴 처리와 같은 경로를 탄다(경로가 둘이면 정책이 갈린다).
      생명주기 스케줄러와 함께 만든다
...
- [ ] **탈퇴 전환 예고 템플릿 신규** — 현재는 기존 `ACCOUNT_DORMANT_TRANSFERRED` 를 쓴다.
      되돌릴 수 없는 전환 전 통지 문구를 따로 만드는 것이 맞다
- [ ] 게시글 작성자 익명화 — `writer_name` 은 그 자체로 개인정보다(§결정 대기)
...
- [ ] 복원 수단 ① **실명인증(NICE)** — jar·JPMS 플래그는 준비됐으나 계약 사이트코드·
      비밀번호가 없어 실측 불가. 코드를 넣어도 검증할 수 없어 미착수.
      복원 처리(`restore`)는 이미 수단과 분리돼 있어 확인 경로만 붙이면 된다
- [ ] 인증번호 발송 자체의 레이트리밋 — 현재는 쿨다운(60초)만. 로그인 레이트리밋과 별개로
      IP 기준 제한을 검토

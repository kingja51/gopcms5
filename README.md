...
1인 개발 프로젝트 · 현재 **구현 단계** — P0 스캐폴딩 ~ P10 회원 관리까지 완료.
사이트 해석·3축 템플릿 Resolver, DB 단일 원천 RBAC(다중 SecurityFilterChain), 3-DB 분리,
관리자 모듈, 파일 업로드·다운로드, 게시판, 회원 생명주기(본인인증·소셜·휴면·탈퇴)가 동작한다.
페이즈별 상세 진행은 [PLAN.md](PLAN.md) 의 체크박스가 정본이다.
...
단계별 체크박스의 정본은 [PLAN.md](PLAN.md) 다 — 여기는 큰 줄기만 둔다.

- [x] eGov 5.0 호환 의존성 설계 (pom)
- [x] KRDS × Tailwind v4 스타일가이드
- [x] 와이어프레임 7종 + 템플릿 Resolver 설계
- [x] Maven 스캐폴딩 + 3-DB/Flyway + `_default` 템플릿 (P0~P1)
- [x] Resolver 파이프라인 + 3축 전환·폴백 통합 테스트 (P2~P5)
- [x] Spring Security — 다중 FilterChain + DB 단일 원천 RBAC (P6)
- [x] 관리자 모듈 + 레이아웃 7종 양산 (P7)
- [x] 파일 업로드·다운로드 (P8) · 게시판 (P9) · 회원 관리 (P10)
- [ ] 부서·직원 모듈
- [ ] 운영 배포 준비 — 관리자 계정 발급 절차, 신뢰 프록시 설정, ClamAV 연동
- [ ] eGovFrame 호환성 확인 신청
